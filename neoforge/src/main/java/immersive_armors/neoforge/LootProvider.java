...
public class LootProvider {
    ...
    private static class DataProvider extends GlobalLootModifierProvider {
        public DataProvider(DataOutput output, CompletableFuture<RegistryWrapper.WrapperLookup> registries, String id) {
        ...
        protected void start() {
            for (String s : Items.lootLookup.keySet()) {
                add("armor_modifier_serializer_" + s, new ImmersiveArmorsLootModifier
                        (new LootCondition[]{
                                LootTableIdCondition.builder(Identifier.of(s)).build()
                        }));
            }
        }
    }
